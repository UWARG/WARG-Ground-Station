...
var TelemetryData = function(){
	this.headers=[];	// Title of each column of data received
	this.current_state= {};		// Current flight state
	this.state_history= [];	// All past flight states (increasing index more recent)
	
	this.received= [];	// All [{time, data}] received from dataRelay (one item per transmission, increasing index more recent)
	this.sent= [];		// All [time, data] data sent to dataRelay (one item per transmission, increasing index more recent)

	this.compIDs= [];	// A list of computer IDs
	this.targets= [];	// All received targets ({lat: float, lon: float, type: enum(1..5), comp: enum(0..2)})
