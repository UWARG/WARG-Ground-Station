var Path = (function ($, Data, Log, Network, Mousetrap, HeightGraph) {
    ...
    $(document).ready(function () {
        if (!map) {
            ...
            if (!scLimits) {
                ...
                scLimits = new L.Polyline(coords, {
                    color: '#00ff00', opacity: 1, weight: 3, clickable: false
                }).addTo(map);
            }

            // Init SFOC box if necessary
            if (!sfocLimits) {
                var coords = [
    ...
    var historyPolyline;
    var scLimits;   // SC box
    var sfocLimits; // SFOC box
    var trainTracks;// Train tracks (KML)
